/**
 * V2PipelineTestHelper - Shared helper for V2 pipeline integration tests
 *
 * Creates test data (files with pipeline_status, upload_batches, file_chunks)
 * and tracks created resources for FK-aware cleanup.
 *
 * @module __tests__/integration/helpers/V2PipelineTestHelper
 */
...
/**
 * Helper for V2 pipeline integration tests.
 * Manages test data creation and FK-aware cleanup.
 */
export class V2PipelineTestHelper {
  ...
  async createTestUser(opts?: { userId?: string; email?: string }): Promise<TestUser> {
    ...
    const email = opts?.email ?? `test-${userId.slice(0, 8)}@v2pipeline-test.local`;
...
/**
 * Factory function to create a new V2PipelineTestHelper instance.
 */
export function createV2PipelineTestHelper(): V2PipelineTestHelper {
  return new V2PipelineTestHelper();
}
