/**
 * FileChunkWorkerV2 Unit Tests (PRD-04)
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { FileChunkWorkerV2 } from '@/infrastructure/queue/workers/v2/FileChunkWorkerV2';
import type { Job } from 'bullmq';
import type { V2ChunkJobData } from '@/infrastructure/queue/workers/v2/FileChunkWorkerV2';
...
const createMockJob = (data: V2ChunkJobData, overrides?: Partial<Job<V2ChunkJobData>>): Job<V2ChunkJobData> => ({
  id: 'test-job-id',
  data,
  attemptsMade: 0,
  name: `chunk:${data.fileId}`,
  ...overrides,
} as unknown as Job<V2ChunkJobData>);

const SAMPLE_JOB_DATA: V2ChunkJobData = {
...
describe('FileChunkWorkerV2', () => {
  let worker: FileChunkWorkerV2;
  ...
  it('should verify state, process chunks, and advance to embedding', async () => {
    ...
    // Verify service called with skipNextStageEnqueue
