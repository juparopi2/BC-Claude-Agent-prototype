/**
 * Unit tests for FileEmbedWorkerV2 (PRD-04)
 *
 * Tests the V2 embedding generation worker that:
...
import {
  FileEmbedWorkerV2,
  type V2EmbedJobData,
  getFileEmbedWorkerV2,
} from '@/infrastructure/queue/workers/v2/FileEmbedWorkerV2';

// Sample test data
const SAMPLE_JOB_DATA: V2EmbedJobData = {
...
describe('FileEmbedWorkerV2', () => {
  let worker: FileEmbedWorkerV2;
  let mockJob: Job<V2EmbedJobData>;

  beforeEach(() => {
    vi.clearAllMocks();

    worker = new FileEmbedWorkerV2({ logger: mockLogger });

    mockJob = {
      id: 'job-123',
      data: SAMPLE_JOB_DATA,
      attemptsMade: 0,
    } as Job<V2EmbedJobData>;
  });
  ...
  describe('Success cases', () => {
    it('should successfully process file with chunks', async () => {
      ...
      // Verify dual-write legacy column
      expect(mockExecuteQuery).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE files SET embedding_status'),
        { fileId: SAMPLE_JOB_DATA.fileId, status: EMBEDDING_STATUS.COMPLETED },
      );
  ...
  describe('Error handling', () => {
    it('should transition to failed state on embedding generation error', async () => {
      ...
      // Verify dual-write legacy failure
      expect(mockExecuteQuery).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE files SET embedding_status'),
        { fileId: SAMPLE_JOB_DATA.fileId, status: EMBEDDING_STATUS.FAILED },
      );
    });
  ...
  describe('Factory function', () => {
    it('should create instance via factory', () => {
      const instance = getFileEmbedWorkerV2({ logger: mockLogger });
      expect(instance).toBeInstanceOf(FileEmbedWorkerV2);
    });

    it('should create instance with default logger', () => {
      const instance = getFileEmbedWorkerV2();
      expect(instance).toBeInstanceOf(FileEmbedWorkerV2);
    });
  });
});
