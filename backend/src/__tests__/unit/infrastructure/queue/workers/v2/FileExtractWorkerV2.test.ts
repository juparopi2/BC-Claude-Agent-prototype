/**
 * FileExtractWorkerV2 Unit Tests (PRD-04)
 */

import { describe, it, expect, vi, beforeEach } from 'vitest';
import { FileExtractWorkerV2 } from '@/infrastructure/queue/workers/v2/FileExtractWorkerV2';
import type { Job } from 'bullmq';
import type { V2ExtractJobData } from '@/infrastructure/queue/workers/v2/FileExtractWorkerV2';
...
const createMockJob = (data: V2ExtractJobData, overrides?: Partial<Job<V2ExtractJobData>>): Job<V2ExtractJobData> => ({
  id: 'test-job-id',
  data,
  attemptsMade: 0,
  name: `extract:${data.fileId}`,
  ...overrides,
} as unknown as Job<V2ExtractJobData>);

const SAMPLE_JOB_DATA: V2ExtractJobData = {
...
describe('FileExtractWorkerV2', () => {
  let worker: FileExtractWorkerV2;
  ...
  beforeEach(() => {
    ...
    worker = new FileExtractWorkerV2({
      logger: {
        info: vi.fn(),
        warn: vi.fn(),
        error: vi.fn(),
