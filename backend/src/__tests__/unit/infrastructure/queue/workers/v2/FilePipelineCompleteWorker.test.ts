...
import { FilePipelineCompleteWorker } from '@/infrastructure/queue/workers/v2/FilePipelineCompleteWorker';
import type { V2PipelineCompleteJobData } from '@/infrastructure/queue/workers/v2/FilePipelineCompleteWorker';
...
describe('FilePipelineCompleteWorker', () => {
  ...
  function createMockJob(data: V2PipelineCompleteJobData): Job<V2PipelineCompleteJobData> {
    return {
      id: 'job-12345',
      data,
    } as Job<V2PipelineCompleteJobData>;
  }

  it('should process file and update batch progress when batch is NOT complete', async () => {
    const jobData: V2PipelineCompleteJobData = {
      fileId: TEST_FILE_ID,
    ...
    expect(mockLogger.info).toHaveBeenCalledWith('V2 pipeline-complete finished');
  });
