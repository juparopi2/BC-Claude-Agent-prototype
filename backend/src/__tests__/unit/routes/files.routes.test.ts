...
const mockMessageQueue = {
  ...
  addFileProcessingFlow: vi.fn().mockResolvedValue({ id: 'FLOW-001' }),
};
...
describe('Files Routes', () => {
  ...
  // ============================================
  // NOTE: POST /api/files/upload route was removed in PRD-07 migration.
  // Upload is now handled via the upload-session workflow.
  // Tests for that route have been removed.

  // ============================================
  // NOTE: POST /api/files/check-duplicates route was removed in PRD-07 migration.
  // Duplicate detection is now part of the upload-session workflow.
  // Tests for that route have been removed.
  ...
  describe('POST /api/files/:id/retry-processing', () => {
    it('should initiate full processing retry', async () => {
      ...
      // Assert - new V2 pipeline returns batchId instead of jobId
      expect(response.body.file).toBeDefined();
      expect(response.body.batchId).toBeDefined();
      expect(response.body.message).toBe('Processing retry initiated');
    });

    it('should initiate embedding-only retry via V2 pipeline', async () => {
      // Act - Both scopes use V2 pipeline (BullMQ Flow)
      const response = await request(app)
        .post(`/api/files/${VALID_FILE_UUID}/retry-processing`)
        .send({ scope: 'embedding_only' })
        .expect(200);

      // Assert - V2 pipeline uses addFileProcessingFlow for all retry scopes
      expect(response.body.batchId).toBeDefined();
      expect(mockMessageQueue.addFileProcessingFlow).toHaveBeenCalled();
    });
    ...
    it('should default to full scope when not specified', async () => {
      ...
      // Assert - V2 pipeline uses addFileProcessingFlow
      expect(mockMessageQueue.addFileProcessingFlow).toHaveBeenCalled();
    });
  ...
  describe('Multi-Tenant Isolation', () => {
    ...
    it('should verify file ownership before single-file delete', async () => {
      // Arrange - FileService verifies ownership on deleteFile
      ...
      // Assert - deleteFile is called with the authenticated userId
