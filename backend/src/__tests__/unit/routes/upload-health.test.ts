 ...
 *
 * Endpoint tested:
 * - GET /api/uploads/health - Pipeline state machine definition + distribution
 *
...
describe('Upload Health Route (PRD-01)', () => {
  ...
  // Success Cases
  // ============================================
  describe('GET /api/uploads/health - Success', () => {
    it('should return 200 with correct response shape', async () => {
      ...
      mockFileRepository.getStatusDistribution.mockResolvedValueOnce(mockDistribution);
    ...
    it('should call getStatusDistribution on the repository', async () => {
      // Arrange
      mockFileRepository.getStatusDistribution.mockResolvedValueOnce({
        registered: 0,
        uploaded: 0,
        queued: 0,
        extracting: 0,
      ...
      // Assert
      expect(mockFileRepository.getStatusDistribution).toHaveBeenCalledTimes(1);
    });
    ...
    it('should handle empty distribution (all zeros)', async () => {
      ...
      mockFileRepository.getStatusDistribution.mockResolvedValueOnce(emptyDistribution);
    ...
    it('should handle large file counts in distribution', async () => {
      ...
      mockFileRepository.getStatusDistribution.mockResolvedValueOnce(largeDistribution);
  ...
  // Error Cases
  // ============================================
  describe('GET /api/uploads/health - Errors', () => {
    it('should return 503 when repository throws an error', async () => {
      // Arrange - Repository throws
      mockFileRepository.getStatusDistribution.mockRejectedValueOnce(
    ...
    it('should return 503 when repository returns undefined', async () => {
      ...
      mockFileRepository.getStatusDistribution.mockResolvedValueOnce(undefined);
  ...
  // ============================================
  describe('GET /api/uploads/health - Authentication', () => {
  ...
  // Edge Cases
  // ============================================
  describe('GET /api/uploads/health - Edge Cases', () => {
    it('should handle concurrent requests', async () => {
      // Arrange
      mockFileRepository.getStatusDistribution.mockResolvedValue({
        registered: 1,
        uploaded: 2,
        queued: 3,
        extracting: 4,
      ...
      const requests = Array.from({ length: 5 }, () =>
        request(app).get('/api/uploads/health')
      );
      ...
      expect(mockFileRepository.getStatusDistribution).toHaveBeenCalledTimes(5);
    });
    ...
    it('should handle rapid sequential requests', async () => {
      ...
      const response1 = await request(app).get('/api/uploads/health');
      const response2 = await request(app).get('/api/uploads/health');
      const response3 = await request(app).get('/api/uploads/health');
  ...
  // ============================================
  describe('GET /api/uploads/health - State Machine Validation', () => {
  ...
  // ============================================
  describe('GET /api/uploads/health - Distribution Validation', () => {
