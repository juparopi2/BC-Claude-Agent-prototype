 ...
 *
 * Floating panel displaying batch upload progress.
 * Supports multiple concurrent batches, each shown as an independent card.
 *
 * @module components/files/BatchUploadProgressPanel
 */
...
import type { BatchEntry } from '@/src/domains/files/stores/uploadBatchStore';
import { computeBatchProgress } from '@/src/domains/files/hooks/useUploadProgress';
import type { UploadProgress } from '@/src/domains/files/hooks/useUploadProgress';
...
interface BatchUploadProgressPanelProps {
...
function BatchProgressCard({ entry, onCancel, onDismiss }: BatchProgressCardProps) {
  ...
  const progress: UploadProgress = useMemo(
...
export function BatchUploadProgressPanel({ onCancel }: BatchUploadProgressPanelProps) {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const batches = useBatchUploadStore(useShallow((s) => s.batches));
  const removeBatchAction = useBatchUploadStore((s) => s.removeBatch);
