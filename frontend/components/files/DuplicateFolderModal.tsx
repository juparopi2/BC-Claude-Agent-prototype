 ...
 *
 * Folder-level duplicate resolution dialog.
 * Offers Cancel, Skip, Replace, and Keep Both actions.
 *
 * @module components/files/DuplicateFolderModal
 */
...
interface FolderDuplicateContentProps {
  ...
  onResolve: (resolution: FolderDuplicateResolution) => void;
  onResolveAll: (action: FolderDuplicateAction) => void;
  onCancel: () => void;
}
      ...
      <div className="space-y-3 py-4 overflow-hidden">
        ...
        {remainingCount > 1 && (
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="apply-all-folder" className="text-sm">
...
export function DuplicateFolderModal() {
  const results = useFolderDuplicateStore((s) => s.results);
  const resolutions = useFolderDuplicateStore((s) => s.resolutions);
  const isModalOpen = useFolderDuplicateStore((s) => s.isModalOpen);
  const targetFolderPath = useFolderDuplicateStore((s) => s.targetFolderPath);
  const resolveOne = useFolderDuplicateStore((s) => s.resolveOne);
  const resolveAllRemaining = useFolderDuplicateStore((s) => s.resolveAllRemaining);
  const cancel = useFolderDuplicateStore((s) => s.cancel);
