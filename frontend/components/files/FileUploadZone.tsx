...
interface FileUploadZoneProps {
  ...
  className?: string;
  /** When true, drops redirect to local files root instead of current folder */
  isCloudView?: boolean;
}
...
export function FileUploadZone({
  ...
  // When viewing cloud files, redirect drops to local root
  const targetFolderId = isCloudView ? null : currentFolderId;
    ...
    try {
      if (dropType === 'folder' || dropType === 'mixed') {
        ...
        const standaloneFiles = structure.validFiles
          ...
          .map((f) => f.file);
        await startUploadV2(standaloneFiles, structure.rootFolders, targetFolderId);
      }
  ...
  }, [targetFolderId, startUploadV2, checkConcurrentLimit]);
  ...
  const onDrop = useCallback(async (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
    ...
    if (acceptedFiles.length > 0) {
      ...
      if (!checkConcurrentLimit()) return;
      await startUploadV2(acceptedFiles, undefined, targetFolderId);
    }
  }, [startUploadV2, targetFolderId, checkConcurrentLimit]);
  ...
  return (
    ...
    >
      ...
      {isDragActive && (
        <div className={cn(
          'absolute inset-0 flex flex-col items-center justify-center backdrop-blur-sm z-10',
          isCloudView ? 'bg-blue-50/80 dark:bg-blue-950/80 ring-2 ring-blue-400/50 ring-inset' : 'bg-background/80'
        )}>
          {isDraggingFolder ? (
            <>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {isCloudView
                  ? 'Files will be saved to your Local Files'
