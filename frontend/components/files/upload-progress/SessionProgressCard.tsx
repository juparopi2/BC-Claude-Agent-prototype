...
/**
 * SessionProgressCard Component
 *
 * Individual upload session progress card with collapsible details.
 * Shows folder-level progress, batch grid, and cancel/retry actions.
 *
 * @module components/files/upload-progress/SessionProgressCard
 */
...
import { useMultiUploadSessionStore } from '@/src/domains/files';
import { cn } from '@/lib/utils';
import type { UploadSession } from '@bc-agent/shared';
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-1 flex-wrap">
                  {session.folderBatches.map((batch, idx) => {
                    ...
                    const batchIsFailed = batch.status === 'failed';
                    return (
                      <div
                        key={batch.tempId}
                        className={cn(
                          ...
                          batchIsFailed && !isCurrentBatch && 'bg-red-100',
                          batch.status === 'pending' && !isCurrentBatch && 'bg-muted',
                          !['completed', 'failed', 'pending'].includes(batch.status) &&
                            !isCurrentBatch && 'bg-muted',
                          batchIsFailed && onRetryFolder && 'cursor-pointer hover:bg-red-200'
                        )}
                        title={`${batch.name}: ${batch.status}${batchIsFailed && onRetryFolder ? ' (click to retry)' : ''}`}
                        onClick={() => {
                          if (batchIsFailed && onRetryFolder) {
                            onRetryFolder(batch.tempId);
                          }
                        }}
                      >
                        {batchIsFailed && onRetryFolder ? (
