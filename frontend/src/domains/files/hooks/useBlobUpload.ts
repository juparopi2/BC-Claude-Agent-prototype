 ...
 * useBlobUpload Hook
 *
 * Wraps Uppy blob upload for batch uploads.
 * Supports multiple concurrent batches via a per-batch Uppy instance Map.
 *
 * @module domains/files/hooks/useBlobUpload
 */
...
/**
 * Hook for uploading files to Azure Blob via Uppy
 * Manages per-batch Uppy instances for concurrent uploads.
 */
export function useBlobUpload() {
  const updateProgress = useBatchUploadStore((s) => s.updateFileUploadProgress);
  const markFailed = useBatchUploadStore((s) => s.markFileFailed);
