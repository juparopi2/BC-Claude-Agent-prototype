 ...
 * Batch Upload Store
 *
 * Zustand store for batch upload state.
 * Supports multiple concurrent batches, each tracked independently.
 *
 * @module domains/files/stores/uploadBatchStore
 */
...
export const useBatchUploadStore = create<BatchUploadState & BatchUploadActions>()(
  (set, get) => ({
    ...initialState,
...
export function resetBatchUploadStore(): void {
  useBatchUploadStore.setState({ batches: new Map(), hasActiveUploads: false });
}
